...
- **Decimal weights.** `displayWeight` now rounds to **2 dp** (so 1.25 survives instead of becoming 1.3); `formatWeight` is the one shared formatter and strips trailing zeros. `NumberStepper` moved the unit into the label and gives the number the full field width (smaller −/+ buttons, `text-xl`, `min-w-0`); the active-log card gives Weight more flex than Reps (`min-w-0` wrappers) — so `12.5` / `127.5` never clip at 360 px. The set **confirm is a full-width "Log set" row** under the steppers (not an inline checkmark) so it can't be clipped on a narrow screen and stays a large one-tap target.
...
### Tab transitions, Calendar & Workout Days (added 2026-06-22) — update6
- **Animated route transitions (§1).** `PageTransition` now springs the incoming page in with a **direction from the bottom-nav tab order** (`TAB_ORDER` → rank; deeper/unknown = forward; right-tab = from-right), falling back to a cross-fade. Coordinates with the existing tab `WaveTap` ripple + the `layoutId="nav-active"` sliding indicator. **Crucial constraint:** a resting `translateX(0)` still creates a containing block, so (a) `Sheet` now renders through a **`createPortal` to `document.body`** (its `fixed` overlay is viewport-relative under any transform; gated by `useHydrated`), (b) the slide is **omitted entirely** (`x` key absent, not `0`) whenever there's no direction — reduced motion, cross-fades, and **`/workout/active`** (its `fixed` PR bar), and (c) the wrapper is `overflow-x-clip` (clip ≠ a scroll container, so page `sticky` headers keep working). Prev pathname tracked via a module var written in an effect (no ref-during-render). Custom scroll restoration deferred — relies on native back/forward.
- **Calendar (§2).** `WorkoutCalendar` (`src/components/calendar/`) is a **List ⇄ Calendar `Segmented` toggle inside History** (no new tab). Month grid with month+year header, weekday row, today ring, **trained days arena-filled** (multi-session count badge), tap → that day's session (or a sheet if >1), tap today/future-empty → opens the Start sheet. Month prev/next + horizontal **swipe** with a slide animation; a current **day-streak** chip. Pure date math added to `src/lib/date.ts` (`monthGrid`/`startOfMonth`/`addMonths`/`weekdayLabels`/`currentDayStreak`, **no date-fns**), unit-tested in `date.test.ts`. Honours `firstDayOfWeek`.
- **Workout Days (§3) = the `Routine` model, presented.** New `lib/repo/routines.ts` (CRUD + `saveRoutine`/`duplicateRoutine`/`startFromRoutine`/`touchRoutinePerformed`); **no new schema** beyond `Routine.lastPerformedAt` (set on `endWorkout`, drives the card label + sort) — no Dexie version bump. `/routines` lists day cards (`RoutineDayCard`: Start · edit-on-tap · duplicate · swipe-to-delete+undo, mirroring History). `RoutineBuilder` (`/routines/new` + `/routines/[id]`): name, **multi-select add** via the new shared **`ExercisePickerSheet`** (filters + "Add N exercises" + create-custom; also replaces the old single-add `AddExerciseSheet` on the active screen), **drag-reorder** via Motion **`Reorder` + `useDragControls`** (grip handle, `dragListener={false}`), and per-exercise sets/rep-range/rest overrides (inherited defaults shown as placeholders).
- **Start flow (§4).** The center **FAB + SideRail button open a global `StartWorkoutSheet`** (`stores/startSheet.ts`, mounted in `AppShell`): start from a saved day (`startFromRoutine` preloads every exercise in order; reps seed from the resolver via a new `ActiveExercise.targetReps`, weights from existing `lastPerformance`) or **Empty workout**. If a session is already active the FAB **resumes** instead — so `StartWorkoutSheet` also fires a one-time global `hydrate()` to keep `status` reliable app-wide, and **`ensureSeeded` gained an in-memory lock** so that concurrent hydrate races can't double-seed. Mid-session add/remove still works (active screen uses the same multi-picker).
- **Nav change.** Mobile bottom nav is now **Home · History · [Start] · Days · Stats** (Timer tab swapped out for Workout Days to keep it symmetric); Timer stays on the desktop SideRail and gains a Home-header button so it's never orphaned. `startFromRoutine`/`addExercises`/`targetReps` added to the active-workout store. Backup `routineSchema` extended with `lastPerformedAt`.

**Deferred (not yet built):** PWA/service worker (next-pwa), scheduling/planned workouts (`PlannedWorkout` future-day calendar planning — model sketched in update6 §2, intentionally skipped), cross-route shared-element (history card → detail `layoutId` morph) + custom per-tab scroll restoration, body measurements & photos, react-hook-form, fully offline (vendored) exercise images, larger external dataset merges (wger/ExerciseDB — infra is ready, dedupe by name+equipment+muscle).