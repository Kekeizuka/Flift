...
**Deferred (not yet built):** PWA/service worker (next-pwa), routines/templates UI, body measurements & photos, plate calculator UI, drag-to-reorder exercises (Motion `Reorder`), react-hook-form. The plate-math helpers already exist in `src/lib/utils.ts`, ready to wire up.